...
## What this brain should upgrade

Collected from the three ingests, ordered by leverage. Status updated
2026-08-10 — see [[wiki/brain-as-data-system]] for what the rebuild changed.

1. ~~**Decay classes are declared but not yet applied** to existing pages.~~
   **DONE 2026-08-10.** The retrofit ran on pre-v0.2 pages and immediately
   found two real defects: a stale entity page and a conclusion resting on a
   failed replication.
2. ~~**Growth limit** (from Hermes): nothing stops a page bloating each
   ingest.~~ **DONE 2026-08-10** — `scripts/lint.sh` warns at 320 lines and
   fails at 400.
3. **Reversible lifecycle for pages** (from Hermes) — *still open.* Lint now
   reports orphans truthfully (the old check was dead code), but no page ever
   ages to stale/archived; nothing is recoverably retired.
4. **Trace-based reflection** (from Hermes + Shepherd) — *still open.*
   `log.md` records *what was decided*, not the reasoning that produced it.
   Honest retro needs the trail, not the verdict.
5. **Declared blast radius per operation** (from Shepherd) — *partially done.*
   `scripts/derive.py` declares in its own header exactly which files it may
   rewrite, and the boundary Action now *proves* rather than asserts that the
   ownership rule held. Neither generalizes to other operations yet.
